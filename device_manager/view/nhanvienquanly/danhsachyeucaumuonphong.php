<?php
    ...
    $ma = $_SESSION['account'];
    $tongkq = demkqtracuumuon($noidung,$ngayTimKiem);
    $item_per_page = 5;
    $currentpage = !empty($_GET['page'])?$_GET['page']:1;
    $offset = ($currentpage-1)*$item_per_page;
    $sotrang = ceil($tongkq/$item_per_page);
    $result = tracuuycmuonphong($offset,$noidung,$ngayTimKiem);
?>
<section id="main-content" class="column column-offset-20">
<h1>
    Danh sách Yêu Cầu Mượn Phòng:
</h1>
    <form action="index.php?act=danhsachyeucaumuonphong" method="post">
        <input  class="fa-search " type="text" name="noidungtimkiemyeucaumuonphong" placeholder="nhập nội dung tìm kiếm" >
    ...
    <?php foreach ($result as $each){ ?>
        <tr class="">
            ...
            <td ><?php $ngay_dang_ky_timestamp = strtotime($each['ngay_dang_ky']);
                echo date("d/m/Y",$ngay_dang_ky_timestamp) ?></td>
            <td ><div class="box">
                ...
                <div id="popup1&mayeucau=<?php echo $each['id_yeu_cau'] ?>" class="overlay">
                    <div class="popup">
                        <form action="index.php?act=xacnhanmuonphong&mayeucau=<?php echo $each['id_yeu_cau'] ?>" method="post">
                            <h1> Xác Nhận Cho Mượn</h1>
                            ...
                            <?php }
                            ?>
                            <input class="submit" type="submit" value="Xác Nhận Cho Mượn" name="muon">
                        </form>
                    </div>
                </div>
            </td>
            <td><a class="button" href="index.php?act=huymuonphong&mayeucau=<?php echo $each['id_yeu_cau'] ?>" style="background-color: #bd2130">Từ Chối</a></td>
        </tr>
    <?php } ?>
</table>
