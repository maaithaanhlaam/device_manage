<?php
    ...
    $ma = $_SESSION['account'];
    $tongkq = demkqtracuutra($noidung,$ngayTimKiem);
    $item_per_page = 5;
    $currentpage = !empty($_GET['page'])?$_GET['page']:1;
    $offset = ($currentpage-1)*$item_per_page;
    $sotrang = ceil($tongkq/$item_per_page);
    $result = tracuuyctraphong($offset, $noidung,$ngayTimKiem);
?>
<section id="main-content" class="column column-offset-20">
<h1>
    Danh sách Yêu Cầu Trả Phòng:
</h1>
    <form action="index.php?act=danhsachyeucautraphong" method="post">
        <input  class="fa-search " type="text" name="noidungtimkiemyeucautraphong" placeholder="nhập nội dung tìm kiếm" >
